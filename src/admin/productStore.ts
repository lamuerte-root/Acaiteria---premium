import{ProductItem}from'../types';import{COMBOS_DATA,PROMOTIONS_DATA}from'../data/products';
...
...COMBOS_DATA.map(p=>({...p,stock:24,active:true})),
...PROMOTIONS_DATA.map(p=>({id:p.id,name:p.title,subtitle:p.tag,category:'promo' as const,price:p.price,originalPrice:p.originalPrice,description:p.description,image:p.image,badge:p.badge,stock:20,active:true}))
];
